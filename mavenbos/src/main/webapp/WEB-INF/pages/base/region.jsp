...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#button-import").upload({
			name:'upload', // <input type="file">的name属性
			action: '${pageContext.request.contextPath}/region_importData.action', // 请求路径 
			onSelect: function(){
				// 当选中文件后，关闭自动提交，进行校验 
				this.autoSubmit = false; 
				// 判断文件名 是否以 .xls或者.xlsx 结尾 
				var regex = /^.*\.xlsx?$/ ;
				if(regex.test(this.filename())){
					// 通过校验 
					this.submit();
				}else{
					$.messager.alert('警告','上传文件格式必须以.xls或者.xlsx结尾','warning');
				}
			},
