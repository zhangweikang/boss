	...
	function openZjAccountAndClient()
	{
		...
		service.queryOpenAccount(param,function(data){
			...
			if(error_no == 0)	//调用成功
			{
				...
				//alert("开立中登股东账户入参:"+JSON.stringify(openStockParam)+"555:"+openStockParam)
				//开立中登股东账号
				service.queryOpenCompyAccount(openStockParam,function(data){
					//alert("开立中登股东账户入参:"+JSON.stringify(openStockParam))
					if(data.error_no == 0)
					{
/*						layerUtils.iLoading(false);
						...
						//modify by xujianhua at 20151215  如果三方存管或者三方支付设置过交易密码、则跳过，如果绑定过三方存管则跳过三方存管步骤
						//是否开通了三方存管或者三方支付标志（1：一定绑定了三方存管，还可能绑定了三方支付	2：只绑定了三方支付3：未绑定三方存管和三方支付）
