...
define("project/scripts/account/backSetPwd",function(require, exports, module){ 
	...
	{
		var pass1 = $(_pageId+"  #input_text_pass1").attr("data-password");
		var pass2 = $(_pageId+"  #input_text_pass2").attr("data-password");
		var pass3 = $(_pageId+"  #input_text_pass3").attr("data-password");
		var pass4 = $(_pageId+"  #input_text_pass4").attr("data-password");
		...
		{
			$(_pageId+"  #input_text_pass3").attr("data-password",pass1);  // 赋值密码值
			$(_pageId+"  #input_text_pass4").attr("data-password",pass1);  // 赋值密码值
		}
		return true;
	}
	...
	function validatePassword(obj)
	{
		...
		if(!validatorUtil.isNumeric(password)){
			
			layerUtils.iMsg(-1,"输入密码有误，请重新输入!");
			return;
		}
		/**
		 * 增加弱密校验
				...
				service.setAccountPwd(tradePasswordParam, function(data){
					...
					if(errorNo==0)	//交易密码设置成功，锁定交易密码设置框
					{
						...
						service.setAccountPwd(fundPasswordParam, function(data){
							...
							{
								appUtils.pageInit("account/setPwd","account/accountSuccess",{});
							}
							else
							{
